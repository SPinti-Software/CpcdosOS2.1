...
namespace cpinti
{
	...
	{	
												...
												unsigned int PID_Parent, const char* NomProcessus)
		{
			...
			unsigned int Resultat = gestionnaire_tache::ajouter_Processus();
		...
		const char* cpinti_get_nom_processus(unsigned int ID_KERNEL, unsigned int PID)
		{
		...
		unsigned int cpinti_get_nombre_processus()
		{
			...
			return gestionnaire_tache::get_NombreProcessus();
		}
