...
unsigned long th_InterlockedIncrement(unsigned long volatile* Addend){ //Not WINAPI?
	showfunc("InterlockedIncrement( volatile: %p )", Addend);
	#ifdef USE_Platform_Atomic
		return InterlockedIncrement(Addend);
	#else
		(*Addend)++; //TODO Atomic operation
		return *Addend;
	#endif
}
//!FORCEINLINE unsigned long InterlockedDecrement (unsigned long volatile *Addend)
unsigned long th_InterlockedDecrement (unsigned long volatile *Addend) { //Not WINAPI?
	showfunc("InterlockedDecrement( volatile: %p )", Addend);
	#ifdef USE_Platform_Atomic
		return InterlockedIncrement(Addend);
	#else
		(*Addend)--; //TODO Atomic operation
